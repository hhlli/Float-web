        ...
        <div class="login-card">
          ...
          <template v-else>
            <div class="form-group" style="margin-bottom: 16px;">
              ...
              <input type="text" v-model="loginForm.code" ref="codeInput" @keydown.enter="doLogin" placeholder="输入 6 位动态验证码" maxlength="6" class="code-input">
            </div>
          </template>
          ...
          <div class="login-actions">
            <button class="btn-outline" style="flex: 1;" @click="handleCancel">取消</button>
            <button class="btn-primary" style="flex: 1; margin-left: 12px;" @click="doLogin" :disabled="isLoading">
              {{ isLoading ? '处理中...' : (need2FA ? '验证并登录' : '登录') }}
            </button>
          </div>
...
const need2FA = ref(false) // 标识是否需要输入 2FA
