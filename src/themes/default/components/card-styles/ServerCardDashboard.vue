...
import { formatBytes, formatSpeed, formatUptime, calculateDaysLeft, getFlagEmoji } from '@/utils/format.js'
...
const gauges = computed(() => {
  ...
  return [
    { label: 'CPU', pct: Math.min(s.cpu || 0, 100), color: getColor(s.cpu || 0) },
    { label: 'RAM', pct: Math.min(s.mem || 0, 100), color: getColor(s.mem || 0) },
    { label: 'Disk', pct: Math.min(s.disk || 0, 100), color: getColor(s.disk || 0) },
    {
      label: 'Swap', 
      pct: s.swap_total ? Math.min(((s.swap_used || 0) / s.swap_total) * 100, 100) : 0,
...
.name-group { display: flex; align-items: center; gap: 8px; min-width: 0; }
.name {
  font-size: 16px;
  font-weight: 700;
  color: var(--text-main);
...
.main-gauges { display: grid; grid-template-columns: repeat(4, 1fr); gap: 12px; }
...
.ring-fill { transition: stroke-dasharray 0.8s cubic-bezier(0.4, 0, 0.2, 1); }
.ring-content { position: absolute; inset: 0; display: flex; align-items: center; justify-content: center; }
.pct-num { font-size: 14px; font-weight: 800; font-variant-numeric: tabular-nums; }
.pct-num small { font-size: 10px; opacity: 0.7; margin-left: 1px; }
.gauge-label {
  font-size: 11px;
  font-weight: 600;
  color: var(--text-muted);
...
.dash-footer { display: flex; flex-direction: column; gap: 12px; }
.traffic-mini { display: flex; flex-direction: column; gap: 4px; }
.traffic-track { height: 4px; background: var(--border-color); border-radius: 2px; overflow: hidden; }
.traffic-fill { height: 100%; transition: width 1s ease; }
.traffic-text { display: flex; justify-content: space-between; font-size: 10px; color: var(--text-muted); font-weight: 500; }
.meta-info {
  display: flex;
  align-items: center;
